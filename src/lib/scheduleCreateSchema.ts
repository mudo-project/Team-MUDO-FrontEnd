...
export const scheduleCreateSchema = z
  .object({
    ...
    startDate: z.string().trim().min(1, "날짜를 선택해주세요."),
    endDate: z.string().trim().min(1, "날짜를 선택해주세요."),
    allDay: z.boolean(),
    startTime: z.string(),
    endTime: z.string(),
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.startDate === values.endDate && values.startTime && values.endTime && values.startTime >= values.endTime) {
