...
interface FinanceCardMonthOverviewProps {
    ...
    approvalProgress: { inProgress: number; approved: number; rejected: number };
    onClickWrite: () => void;
}
        ...
        >
            <div className="flex flex-col justify-center border-r border-[#E1EBE3] px-5">
                ...
                <strong className="mt-1 block text-[28px] leading-none tracking-[-0.04em]">
                    {totalAmount.toLocaleString()} <span className="text-[13px]">원</span>
                </strong>
            </div>
            <div className="flex flex-col justify-center border-r border-[#E1EBE3] px-5">
                <span className="text-[11px] text-[#718096]">사용 건수</span>
                <strong className="mt-1 block text-[20px] leading-none">{usageCount}건</strong>
            </div>
            <div className="flex flex-col justify-center border-r border-[#E1EBE3] px-5">
                <span className="text-[11px] text-[#718096]">사용목적 미작성</span>
                <button
                    className="mt-1 flex items-center gap-0.5 text-[20px] font-bold leading-none text-[#B78236]"
                    onClick={onClickWrite}
                    type="button"
                >
                    {unwrittenCount}건
                    <span className="ml-1.5 flex items-center text-[12px] font-medium text-[#94A3B8]">
                        작성하기 <ChevronRight className="size-3" />
                    </span>
                </button>
            </div>
            <div className="flex flex-col justify-center px-5">
                <span className="text-[11px] text-[#718096]">결재 진행</span>
                <p className="mt-1.5 flex items-center gap-2 text-[11px] text-[#718096]">
                    <span className="flex items-center gap-1">
                        <span className="size-1.5 rounded-full bg-[#2B6CB0]" />진행중 {approvalProgress.inProgress}
                    </span>
                    <span className="flex items-center gap-1">
                        <span className="size-1.5 rounded-full bg-[#2F7D46]" />승인됨 {approvalProgress.approved}
                    </span>
                    <span className="flex items-center gap-1">
                        <span className="size-1.5 rounded-full bg-[#C0392B]" />반려됨 {approvalProgress.rejected}
                    </span>
                </p>
            </div>
        </section>
