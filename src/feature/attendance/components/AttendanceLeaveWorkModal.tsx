...
import { formatDateWithWeekday, formatDuration, formatTime, type AttendanceRecordState } from "../attendanceDemo";
...
export default function AttendanceLeaveWorkModal({ now, record, onCancel, onConfirm }: AttendanceLeaveWorkModalProps) {
  const [note, setNote] = useState("");
  const workedMs = record.clockInAt ? now.getTime() - record.clockInAt.getTime() : 0;
    ...
    >
      <div className="w-full max-w-[420px] rounded-2xl bg-white p-6">
        ...
        <div className="mt-4 rounded-lg bg-[#F8FAFC] px-4 py-3 text-[13px] text-[#344054]">
          {formatDateWithWeekday(now)} {formatTime(now)}
        </div>

        <div className="mt-4 space-y-2 rounded-lg bg-[#F8FAFC] px-4 py-3 text-[13px]">
          <div className="flex items-center justify-between">
            <span className="text-[#718096]">출근</span>
            <span className="font-medium text-[#172033]">{record.clockInAt ? formatTime(record.clockInAt) : "--:--"}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[#718096]">퇴근</span>
            <span className="font-medium text-[#172033]">{formatTime(now)}</span>
          </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          ...
          <button 
            className="h-10 rounded-lg bg-[#172033] px-4 text-[13px] font-semibold text-white" 
            type="button" 
