...
import { changeWorkspaceRecurringTemplateAction } from "../../actions";
...
export default function EditTaskTemplateModal({
    closeModal,
    workspaceId,
    template,
    ...
    const defaultRepeat = template.recurrenceType === "MONTHLY"
        ? "MONTHLY"
        : `WEEKLY${"daysOfWeek" in template.recurrenceRule
            ? template.recurrenceRule.daysOfWeek[0] ?? 1
            : 1}`;
        ...
        <div className="fixed top-0 left-0 z-999 h-screen w-screen bg-[#162236]/30"
            ...
            <form
                onSubmit={(event) => {
                    ...
                    setError("");
                    editMutation.mutate(new FormData(event.currentTarget));
                }}
                ...
                <button
                    ...
                    type="submit"
                >
                    {editMutation.isPending ? "수정 중..." : "템플릿 수정"}
                </button>
            </form>
        </div>
    );
