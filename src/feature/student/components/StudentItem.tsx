    ...
    return (
        <>
            <button
                ...
                data-testid={`student-list-row-${student.id}`}
                onClick={viewStudentModal.openModal}
                type="button"
            >
                <p className="col-span-1 text-[#CBD5E1]">{student.id}</p>
                ...
                <p className="col-span-1">{student.grade}</p>
                <p className="col-span-2 whitespace-nowrap">{student.phone}</p>
                <p className="col-span-2 whitespace-nowrap">{student.guardianPhone}</p>
                <p className="col-span-1 font-semibold text-[#3D7A6A]">{student.courseCount}개</p>
                <p className="col-span-2 truncate text-[11px]">{student.school}</p>
            </button>

            {viewStudentModal.isModal && (
                <ViewStudentModal closeModal={viewStudentModal.closeModal} />
            )}
        </>
    );
}
