...
export default function CreateStudentModal({ closeModal }: { closeModal: () => void }) {
    ...
    return (
        <div className="fixed top-0 left-0 z-999 flex h-screen w-screen items-center justify-center bg-black/35" onClick={closeModal}>
            <form className="w-[460px] rounded-[14px] bg-white px-7 pt-7 pb-6 shadow-[0_8px_20px_rgba(0,0,0,0.18)]" onClick={(event) => event.stopPropagation()} onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="mt-[22px] flex justify-end gap-2">
                    ...
                    <button
                        className="h-[40px] rounded-[8px] bg-[#2A3A4A] px-5 text-[13px] leading-[19.5px] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
