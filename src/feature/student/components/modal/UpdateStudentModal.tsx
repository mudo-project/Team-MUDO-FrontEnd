...
export default function UpdateStudentModal({ closeModal, onUpdated, student, studentId }: UpdateStudentModalProps) {
    ...
    const onSubmit = async (data: CreateStudentFormValues) => {
        try {
            const response = await updateStudentAction(studentId, {
                name: data.name,
                grade: data.grade,
    ...
    return (
        <div className="fixed top-0 left-0 z-1001 flex h-screen w-screen items-center justify-center bg-black/35" onClick={closeModal}>
            <form className="relative z-1002 w-[460px] rounded-[14px] bg-white px-7 pt-7 pb-6 shadow-[0_8px_20px_rgba(0,0,0,0.18)]" onClick={(event) => event.stopPropagation()} onSubmit={handleSubmit(onSubmit)}>
                <h2 className="text-[16px] leading-6 font-bold text-[#1D2B3A]">원생 정보 수정</h2>
                ...
                <div className="mt-5 flex w-full flex-col gap-[13px]">
                    ...
                    <label className="relative text-[12px] leading-[18px] font-medium text-[#64748B]">
                        ...
                        <select {...register("grade")} className="mt-[5px] h-[38px] w-full appearance-none rounded-[8px] border border-[#DCE8E2] bg-white px-4 text-[13px] text-[#1D2B3A] outline-none">
                        ...
                        <ChevronDown aria-hidden="true" className="pointer-events-none absolute right-3 bottom-[13px] size-3 text-[#64748B]" strokeWidth={1.5} />
                    </label>

                    <label className="text-[12px] leading-[18px] font-medium text-[#64748B]">
                        특이사항
                        <textarea {...register("note")} className="mt-[5px] h-[78px] w-full resize-none rounded-[8px] border border-[#DCE8E2] bg-white px-3 py-2 text-[13px] text-[#1D2B3A] outline-none" maxLength={500} />
                ...
                <div className="mt-[22px] flex justify-end gap-2">
                    <button className="h-[40px] rounded-[8px] border border-[#DCE8E2] bg-white px-[18px] text-[13px] leading-[19.5px] text-[#64748B]" onClick={closeModal} type="button">취소</button>
                    <button className="h-[40px] rounded-[8px] bg-[#2A3A4A] px-5 text-[13px] leading-[19.5px] font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={isSubmitting} type="submit">
