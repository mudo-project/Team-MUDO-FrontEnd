...
- 구글 계정 연동/재연결/계정 교체는 모두 `getGoogleAuthorizationUrlAction`으로 받은 `authorizationUrl`을 `window.open`으로 팝업을 띄우고, 500ms 간격으로 `popup.closed`를 폴링하다가 팝업이 닫히면 `getGoogleConnectionAction`을 다시 호출해 최신 상태를 반영하는 방식으로 동작한다. 팝업이 실제로 인증에 성공했는지는 구분하지 않고, "팝업이 닫힌 뒤의 서버 상태"만 신뢰한다(팝업 안에서 postMessage 등으로 결과를 알려주는 절차는 없음).
...
- `SettingGoogleConnectionCallback`(위 브리지 페이지의 본문)은 마운트 즉시 `window.close()`부터 시도한다(성공/실패 여부와 무관하게 화면에 아무것도 그리지 않음 — 결과 안내는 이미 열려 있는 `/setting/google`이 보여주는 게 맞다고 판단해 팝업 자체에는 안내 문구를 두지 않았다). `window.close()`가 실패했을 때만(스크립트로 열리지 않은 창 등) 300ms 후 성공/실패 문구 + `창 닫기` 버튼을 보여주는 최소한의 폴백만 있다. 메인 창의 폴링(`popup.closed` 감지)은 이 닫힘에 의존한다.
...
| 기능 | 트리거 | 동작 | 상태 |
|---|---|---|---|
| 연동 상태 표시 | 설정 화면 진입 | 관리 버튼 좌측에 "연결됨"/"연결되지 않음" 문구 표시 | 미구현 — `SettingGoogle`의 "연결됨" 배지가 여전히 고정 표시됨. 실제 상태는 `/setting/google`에 들어가야만 확인 가능 |
...
| 구글 계정 연결 | 미연동 화면의 `구글 계정 연결` 클릭 | 구글 인가 URL을 팝업으로 열어 연동 진행 | 구현 완료 — `getGoogleAuthorizationUrlAction(false)`로 `authorizationUrl`을 받아 팝업으로 열고, 500ms 간격으로 팝업이 닫혔는지 폴링하다가 닫히면 `getGoogleConnectionAction`을 다시 호출해 최신 상태로 갱신한다(팝업 내부 진행 결과를 직접 알려주는 절차는 없음) |
| 연동 상세 정보 표시 | 연동됨 상태일 때 | 이메일, 연결됨 여부, 연결 일시, 연결한 관리자, 권한 범위, 토큰 확인 표시 | 구현 완료 — `connection`의 실제 값을 표시. `연결 일시`/`토큰 확인`은 `formatGoogleDateTime()`으로 포맷, `권한 범위`는 서버가 내려주는 raw OAuth scope 문자열을 그대로 표시, `연결한 관리자`는 이름 조회 API가 없어 "사용자 #{connectedByUserId}"로 표시 |
...
| 계정 교체 - 인증 | 모달 2단계 | 스피너, "구글 로그인 창에서 계속 진행해주세요" 안내, "창이 열리지 않았나요? 다시 열기" 버튼 표시. 다시 열기 클릭 시 구글 로그인 창 재오픈 | 구현 완료 — 발급받은 `authorizationUrl`로 팝업을 열고 500ms 간격으로 `popup.closed`를 폴링한다. "다시 열기"는 같은 URL로 팝업을 다시 연다. 팝업이 닫히면 `getGoogleConnectionAction`을 호출해 3단계로 이동 |
...
| 토큰 만료 임박 안내 | 서버 `status`가 `EXPIRING`(만료 3일 전부터) | 상세정보 박스 위에 경고 배너 표시("n일 뒤 토큰이 만료됩니다" / "계정 재연결을 진행해주세요" + 우측 `재연결 →` 바로가기), 박스 안 배지는 "갱신 필요"(주황) | 구현 완료 — `n`은 `connection.tokenExpiresAt`과 현재 시각의 차이를 일 단위로 계산한 실제 값. 배너/박스 안 바로가기 모두 미연동 화면과 동일한 `handleConnect`(재연결, `switchAccount=false`)를 호출한다 |
...
| `formatGoogleDateTime()` | 구글 연동 응답의 ISO 시각(`connectedAt`/`tokenExpiresAt`/`lastCheckedAt`)을 `"YYYY.MM.DD HH:mm"`로 변환하는 함수. `SettingGoogleConnection`에서 사용 |
