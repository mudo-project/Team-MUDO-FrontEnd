...
import ScheduleItem from "@/feature/lecture/components/ScheduleItem";
import { LECTURE_CLASS_TYPE_LABEL, LECTURE_DAY_LABEL, LECTURE_GRADE_LABEL } from "@/feature/lecture/constants";
import ViewRollbookModal from "@/feature/rollbook/components/modals/ViewRollbookModal";
...
export default function RollLectureItem({ lecture }: { lecture: LectureListItemData }) {
        ...
        <>
            <button className="grid min-h-[52px] md:min-h-[56px] lg:min-h-[59px] w-full grid-cols-9 md:grid-cols-13 items-center border-b border-[#F7F8F9] px-3 md:px-4 lg:px-5 py-2 md:py-2.5 lg:py-2.5 text-left last:border-b-0 hover:bg-[#FAFBFA]" onClick={detailModal.openModal} type="button">
                <span className="col-span-3">
                    <strong className="block text-[12px] md:text-[13px] leading-[18px] md:leading-[19.5px] font-medium text-[#0F172A]">{lecture.name}</strong>
