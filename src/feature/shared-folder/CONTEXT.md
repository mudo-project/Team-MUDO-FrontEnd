...
루트가 준비되면 `GET /api/shared-files/items`로 폴더 목록을 실제로 조회하고, 폴더 진입·breadcrumb 이동·더 보기(페이지네이션)·새 폴더 생성·Google Docs/Sheets/Slides 생성·파일 업로드·이름 변경·이동·삭제·다운로드·미리보기 열기까지 실제 API로 동작한다. 상세 보기와 파일 이름 클릭 미리보기는 아직 이 화면에 연결되지 않았다 — 케밥 메뉴에서 상세 보기를 클릭하면 메뉴만 닫힌다.
...
- 이동은 하위 폴더로 들어간 뒤에만 그 위치로 이동할 수 있다. API 명세상 `parentId`를 생략하면 "이동하지 않음"으로 처리되어 공유파일 루트 자체로는 이동할 수 없다(핵심 제약의 루트 id 문제와 동일한 원인).
- 삭제는 Google Drive 휴지통으로 이동하는 것이며 API가 하위 항목 개수를 내려주지 않아, 삭제 확인 모달에는 하위 항목 개수를 표시하지 않는다. 성공하면 목록을 다시 불러온다(경로 이동이 없으므로).
- 다운로드는 원본 형식 그대로만 지원한다. API가 `format`(PDF/DOCX/XLSX/PPTX) 변환 다운로드를 지원하지만, 형식 선택 UI는 아직 제공하지 않는다.
- 다운로드는 서버 응답(바이너리)을 base64 문자열로 감싸 Server Action으로 반환한 뒤, 클라이언트에서 Blob으로 복원해 브라우저 다운로드를 트리거한다.
- 미리보기 열기는 별도 확인 모달 없이 항목의 `viewUrl`을 곧바로 새 탭으로 연다(Google 파일 생성 직후의 "새 탭에서 열기" 모달과 달리 확인 절차가 없다).
...
| **케밥 메뉴** | 내용 행 우측의 `⋮` 버튼과 그 하위 메뉴. 폴더는 이름 변경/이동/삭제, 파일은 상세 보기/미리보기 열기/다운로드/이름 변경/이동/삭제 — 상세 보기를 제외한 나머지는 모두 실제로 동작한다 |
...
| 이동 | 메뉴 `이동` → 모달에서 하위 폴더로 들어가 `이동` 클릭(루트 상태에서는 버튼 비활성) | `PATCH /api/shared-files/items/{itemId}`(`parentId`)으로 이동, 성공 시 목록을 다시 불러온다 | 구현 완료 |
...
| 다운로드 | 메뉴 `다운로드` 클릭 | `GET /api/shared-files/items/{itemId}/download` 호출(원본 형식), 성공 시 브라우저 다운로드를 실행한다. 실패 시 토스트로 오류 안내 | 구현 완료 — 변환 다운로드(`format`)는 미제공 |
| 상세 보기 | 메뉴 `상세 보기` 클릭 | 동작 실행 | 미구현 — 클릭하면 메뉴만 닫힌다 |
...
| 다운로드 가능 여부 | `downloadable` | 화면에서 별도로 사용하지 않음(다운로드 가능 여부를 UI에서 가리지 않고, 실패 시 서버 오류 메시지에 의존한다) |
...
| 컴포넌트 | 책임 |
|---|---|
| **SharedFolderBoard** | 화면 최상위 컴포넌트(client). 마운트 시 시스템 루트 상태(`rootStatus`)를 확인해 사용 불가능하면 안내 화면만 보여준다. 준비되면 목록(`items`)과 로딩·오류·페이지네이션 상태, 현재 경로(`path`), 필터·검색어, 새로만들기·케밥 메뉴 열림 상태, 새 폴더 만들기·Google 파일 만들기·새 탭에서 열기·이름 변경·이동·삭제 모달의 열림 상태(`renamingItem`/`movingItem`/`deletingItem`)와 제출 상태(`isSubmitting`)를 소유한다. 숨겨진 `input[type=file]`(`fileInputRef`)도 여기서 관리한다. 마운트·경로 변경 시 목록을 자동 조회하는 effect는 `.then/.catch/.finally` 안에서만 state를 바꾼다(effect 안에서 곧바로 setState하지 않기 위함). 새 폴더 생성 성공 시에는 생성된 폴더로 바로 이동하고(경로가 바뀌며 자동 조회 effect가 다시 실행됨), Google 파일 생성·파일 업로드·이름 변경·이동·삭제 성공 시에는 경로 이동이 없어 `refreshItems`로 같은 경로를 직접 다시 조회한다. 미리보기 열기는 확인 없이 바로 `viewUrl`을 새 탭으로 열고, 다운로드는 base64 응답을 Blob으로 복원해 브라우저 다운로드를 트리거한다. 상세보기 등 나머지 케밥 메뉴 항목 클릭은 아직 메뉴를 닫기만 한다 |
...
| **SharedFolderItemMenu** | 케밥 메뉴 내용. `kind`가 `FILE`이면 상세 보기/미리보기 열기/다운로드 항목이 추가된다. 상세 보기를 제외한 나머지 항목은 모두 실제로 동작한다 |
...
| 이동 | 케밥 메뉴의 `이동` | 구현 완료 — 공유파일 루트로의 이동은 API 제약상 미지원 |
| 삭제 | 케밥 메뉴의 `삭제` | 구현 완료 |
| 미리보기 열기 · 다운로드 | 케밥 메뉴의 해당 항목 | 구현 완료 — 다운로드는 원본 형식만 지원 |
| 파일 미리보기(파일 이름 클릭) · 상세보기 | 내용 행의 파일 이름 클릭, 케밥 메뉴의 `상세 보기` | 미구현 |
