...
루트가 준비되면 `GET /api/shared-files/items`로 폴더 목록을 실제로 조회하고, 폴더 진입·breadcrumb 이동·더 보기(페이지네이션)·새 폴더 생성·Google Docs/Sheets/Slides 생성·파일 업로드·이름 변경·이동·삭제까지 실제 API로 동작한다. 다운로드는 아직 이 화면에 연결되지 않았다 — 케밥 메뉴는 열리고 닫히지만 클릭해도 실제로 아무 것도 바꾸지 않는다(후속 PR에서 연동 예정).
...
| **케밥 메뉴** | 내용 행 우측의 `⋮` 버튼과 그 하위 메뉴. 폴더는 이름 변경/이동/삭제, 파일은 상세 보기/미리보기 열기/다운로드/이름 변경/이동/삭제 — 이름 변경·이동·삭제는 실제로 동작하고 나머지는 아직 클릭해도 동작하지 않는다 |
...
| 상세 보기 / 미리보기 열기 / 다운로드 | 메뉴에서 각 항목 클릭 | 각 동작 실행 | 미구현 — 클릭하면 메뉴만 닫힌다 |
...
| 컴포넌트 | 책임 |
|---|---|
| **SharedFolderBoard** | 화면 최상위 컴포넌트(client). 마운트 시 시스템 루트 상태(`rootStatus`)를 확인해 사용 불가능하면 안내 화면만 보여준다. 준비되면 목록(`items`)과 로딩·오류·페이지네이션 상태, 현재 경로(`path`), 필터·검색어, 새로만들기·케밥 메뉴 열림 상태, 새 폴더 만들기·Google 파일 만들기·새 탭에서 열기·이름 변경·이동·삭제 모달의 열림 상태(`renamingItem`/`movingItem`/`deletingItem`)와 제출 상태(`isSubmitting`)를 소유한다. 숨겨진 `input[type=file]`(`fileInputRef`)도 여기서 관리한다. 마운트·경로 변경 시 목록을 자동 조회하는 effect는 `.then/.catch/.finally` 안에서만 state를 바꾼다(effect 안에서 곧바로 setState하지 않기 위함). 새 폴더 생성 성공 시에는 생성된 폴더로 바로 이동하고(경로가 바뀌며 자동 조회 effect가 다시 실행됨), Google 파일 생성·파일 업로드·이름 변경·이동·삭제 성공 시에는 경로 이동이 없어 `refreshItems`로 같은 경로를 직접 다시 조회한다. 상세보기/미리보기/다운로드 등 케밥 메뉴의 나머지 항목 클릭은 아직 메뉴를 닫기만 한다 |
...
| **SharedFolderItemMenu** | 케밥 메뉴 내용. `kind`가 `FILE`이면 상세 보기/미리보기 열기/다운로드 항목이 추가된다. 이름 변경·이동·삭제는 실제 모달로 이어지고, 나머지 항목 클릭은 아직 메뉴를 닫기만 한다 |
