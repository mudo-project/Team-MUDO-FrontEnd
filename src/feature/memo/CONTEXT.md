...
> 구현 상태: **부분 구현**. 컨테이너 열림/닫힘·영속성(4.1)은 Zustand(`useMemoStore`)로 구현되어 있다. 메모 CRUD(4.2), 정렬(4.3), 케밥 메뉴(4.4)는 아직 더미 데이터를 렌더링하는 정적 프로토타입 상태다.
...
| 컴포넌트 | 책임 |
|---|---|
| **MemoContainer** | 메모 컨테이너 셸. `layout.tsx`에서 라우트와 무관하게 항상 마운트되며, `useMemoStore`의 `isOpen`을 구독해 열림·닫힘을 결정(구현 완료). Header / Nav / Main 배치를 담당. Header는 별도 컴포넌트로 분리하지 않고 이 안에 직접 작성 |
...
| 메모 데이터 | 메모 목록 조회 및 CRUD | 미구현 (더미 데이터) |
