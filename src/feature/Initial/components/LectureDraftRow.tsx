...
export default function LectureDraftRow({ row, onToggle }: { row: DataImportLectureDraftData; onToggle: (rowId: string) => void }) {
    return (
        <div className="grid grid-cols-12 items-center gap-3 border-t border-[#EEF2EF] px-5 py-3 text-[12px]">
            ...
            <div className="col-span-3"><strong>{row.name}</strong><p className="text-[#94A3B8]">{row.subjectName} · {row.termName}</p></div>
            <span className="col-span-2">{row.teacherName || "-"}</span><span className="col-span-2">{row.classroomName || "-"}</span>
